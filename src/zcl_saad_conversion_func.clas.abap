CLASS zcl_saad_conversion_func DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_saad_conversion_func IMPLEMENTATION.
...
*********************************************************************
    ...
    DATA(today) = CONV d( '20240110' ).". "cl_abap_context_info=>get_system_date(  ).
    ...
    SELECT FROM /dmo/travel
         FIELDS total_price,
          ...
          WHERE currency_code <> 'EUR' AND currency_code = 'USD'
           INTO TABLE @DATA(result_currency).

*"customer_id = '000246' AND
